...
public class PunktyServiceImpl implements IPunktyService {
    ...
    @Autowired
    public PunktyServiceImpl(IPunktyRepository iPunktyRepository, IUzytkownikRepository iUzytkownikRepository, ITrybRepository iTrybRepository){
    ...
    public List<PunktyZapiszDTO> znajdzPunktyPoTrybie2(Long aIdTryb){
        ...
        Collections.sort(listaWynikowaPunktyDTO, (PunktyZapiszDTO a, PunktyZapiszDTO b) -> b.getPunkty().compareTo(a.getPunkty()));
        return listaWynikowaPunktyDTO;
    }
    ...
    public PunktyDTO zapiszPunkty(PunktyDTO aPunktyDTO) throws MyServerException{
        ...
        if(pPunktyOB == null) {
            aPunktyDTO.setTechDate(aPunktyDTO.getTechDate());
            aPunktyDTO.setPunkty(aPunktyDTO.getPunkty());
    ...
    public PunktyDTO zapiszPunkty2(PunktyZapiszDTO aPunktyDTO) throws MyServerException{
        ...
        PunktyOB pPunktyOB = aPunktyDTO.getId() == null ? null :
                iPunktyRepository.findOne(aPunktyDTO.getId());
        if(pPunktyOB == null) {
            bPunktyDTO.setTechDate(aPunktyDTO.getTechDate());
            bPunktyDTO.setPunkty(aPunktyDTO.getPunkty());
